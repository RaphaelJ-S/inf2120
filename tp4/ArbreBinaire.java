...
/**
 *
 * Classe d'objets reprï¿½sentant un arbre binaire de recherche.
...
public class ArbreBinaire< Clef extends Comparable< Clef > > {
    ...
    protected class Noeud< Clef > {
         ...
         * Si une rï¿½fï¿½rence est ï¿½ null, alors le sous arbre
         * est vide.
     ...
     * Un Noeud qui n'a pas d'enfant a une hauteur de 1.
     ...
     * 
     * L'algorithme commence par verifier si l'arbre est vide. Si il ne l'est 
     * pas, l'algorithme utilise la methode rechercheNoeud pour trouver 
     * l'emplacement de la cible puisque ce qui est au dessus du Noeud cherche 
     * n'est pas important. Une fois l'emplacement trouvé, l'algorithme s'assure
     * que la cible est bien dans l'arbre binaire et si c'est le cas, il regarde
     * vers quel(s) Noeuds elle pointe. Dependant du nombre de Noeuds rattaches
     * a la cible, la methode va calculer, soit la hauteur des deux enfants et 
     * prendre la plus eleve des deux, soit la hauteur de l'enfant ou soit 
     * ajouter 1 a la hauteur si elle n'a pas d'enfants. Il est a noter que le 
     * calcul se fait recursivement, donc que ce calcul s'effectue sur chaque
     * Noeud en dessous de la cible jusqu'a ce que l'algorithme tombe sur une
     * feuille, un Noeud sans enfants.
     * 
     * ************************************************************************
     *
     * @param cible La clef reprÃ©sentant le Noeud de l'arbre dont nous voulons
    ...
    public int hauteur( Clef cible ) {
        int hauteur = 0;
        if(_racine != null) {
	        Pair<Noeud<Clef>, Noeud<Clef>> initial = rechercheNoeud(cible);
	        
	    	if (!(initial.premier != null && initial.deuxieme == null) 
	    			|| initial.premier == null && initial.deuxieme == null) {
    		
    			if(initial.deuxieme._droite == null 
    					&& initial.deuxieme._gauche == null) {
    				
    				hauteur += 1;
    				
    			}else if (initial.deuxieme._droite != null 
    					&& initial.deuxieme._gauche != null) {
    				
    				int hautDroit = hauteur(initial.deuxieme._droite._clef);
    				int hautGauche = hauteur(initial.deuxieme._gauche._clef);
    				
    				hauteur += hautDroit > hautGauche ? 
    						hautDroit + 1 : hautGauche + 1;
    						
    			}else {
    				if (initial.deuxieme._gauche == null) {
    					hauteur += hauteur(initial.deuxieme._droite._clef) + 1;
    					
    				}else if (initial.deuxieme._droite == null) {
    					hauteur += hauteur(initial.deuxieme._gauche._clef) + 1; 
    				}
    			}
